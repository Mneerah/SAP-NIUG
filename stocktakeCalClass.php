 ...
 //------------------------WEEK TABLE FUNCTION----------------------------       
    function WeekTable ($diaMes, $diaSemana, $numDiasMes, $nombreMes, $dia, $mes, $ano) {
        ...
        for ($i=1; $i < $diaSemana; $i++) {
            ...
            $Output .=
                        '<div class="calday"> <span class="daylable"> <b>'.$this->dayName ($i).'</b><br>'
                        ...
                        $BodyCal .=
                            "<span id='".$diaMes."-".$mesVar."-".$anoVar."' class='daylong' ondrop='drop(event)'
 ...
 //-----------------------count staff in a day----------------------------
    function countStaff($day, $month, $year) {
        ...
        require ("db_connect.php"); 
            ...
            $sql = "SELECT COUNT(`Staff_idStaff`) AS num 
                    FROM `staffcalendar` where day='$fullDate'";
    ...
    function CallStocktakes ($day, $month, $year) {
            ...
            if (mysqli_num_rows($result)>=0) {
                ...
                while($row = mysqli_fetch_assoc($result)) {
                    if($row["date"]==$fullDate && $row["status"] !="New"){
                        $currentjobs.= '<label id="'. $row["idStocktake"].'"
